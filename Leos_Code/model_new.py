...
class FactorizedOutputHead(nn.Module):
    ...
    def forward(self, h):
        ...
        logits_3d = torch.einsum(       # ((42*32)x32)
            "bshd,bsid,bsjd->bshij",
            pt_emb, eta_emb, phi_emb
        ) 
...
class JetTransformer(nn.Module):
    ...
    def forward(self,
        ...
        mask = x < 0
        x = torch.where(mask, torch.tensor(self.PAD_BIN, device=x.device), x)
    ...
    def sample(
        ...
        for _ in range(max_length):
            ...
            #sample the next id
            # Top-k Werte und Indizes holen
            topk_probs, topk_indices = torch.topk(probs, topk, dim=-1)

            # Wahrscheinlichkeiten neu normalisieren (wichtig!)
            topk_probs = topk_probs / topk_probs.sum(dim=-1, keepdim=True)

            # Aus Top-k sampeln
            next_token_in_topk = torch.multinomial(topk_probs, num_samples=1)

            # Zurück auf Original-Vokabular-Indizes mappen
            next_ids = topk_indices.gather(-1, next_token_in_topk).squeeze(-1)
